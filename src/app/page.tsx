...
export default function CoachAI() {
  ...
  const [yardageDialog, setYardageDialog] = useState<{open:boolean, play:any, yards:string}>({ open: false, play: null, yards: "0" });
  ...
  const refreshPlays = async () => {
    ...
    if (error) toast.error(error.message); else setPlays(data || []);
  };

  const refreshGames = async () => {
    const { data, error } = await supabase.from("games").select("id,opponent,game_date").order("game_date", { ascending: false });
  ...
  const signUp = async () => {
  if (!email || !password || !teamName) return toast.error("Email, password, and team name are required");
  const { data, error } = await supabase.auth.signUp({
    email,
    password,
    // If you later add the trigger approach (below), you can pass team_name here too.
  ...
  // Create or update the profile now that we have an authenticated session.
  const { error: pErr } = await supabase
    .from("profiles")
    .upsert({ user_id: user.id, team_name: teamName || "My Team" }, { onConflict: "user_id" });

  if (pErr) {
    // Not fatal for using the app, but good to surface
...
const fixCallPlay = async (callId: string, newPlayId: string) => {
  ...
  const addGame = async () => {
    ...
    const { data, error } = await supabase.from("games").insert({ opponent: newOpponent.trim(), game_date: newDate }).select("id").single();
    if (error) return toast.error(error.message);
    setNewOpponent(""); toast.success("Game created"); await refreshGames();
  ...
  const perPlayStats = useMemo(() => {
    ...
    for (const r of recentCalls) {
      const k = r.play_name;
      if (!m.has(k)) m.set(k, { count: 0, sum: 0, icon: r.icon });
      const v = m.get(k)!; v.count += 1; v.sum += r.yards;
    }
    return Array.from(m.entries()).map(([name, v]) => ({ name, count: v.count, avg: (v.sum / v.count).toFixed(1), icon: v.icon }));
  }, [recentCalls]);
 ...
 Coach.AI — Team Login
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-2">
              <Label>Email</Label>
              <Input value={email} onChange={e => setEmail(e.target.value)} placeholder="coach@team.com" />
            </div>
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              ...
              <Input value={teamName} onChange={e => setTeamName(e.target.value)} placeholder="Ann Arbor Eagles" />
            </div>
  ...
  return (
    <div className="min-h-screen bg-white p-4 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 font-semibold"><Trophy className="w-5 h-5" /> Coach.AI</div>
        <Button size="sm" variant="outline" onClick={signOut}><LogOut className="w-4 h-4 mr-1"/>Logout</Button>
      </div>

      {/* Plays */}
      <Card className="mb-4">
        <CardHeader><CardTitle>Your Plays</CardTitle></CardHeader>
        <CardContent>
          <div className="flex gap-2 mb-3">
            <Input className="flex-1" placeholder="e.g., Trips Right Flood" value={newPlayName} onChange={e => setNewPlayName(e.target.value)} />
            <Select value={newPlayIcon} onValueChange={setNewPlayIcon}>
              <SelectTrigger className="w-[150px]"><SelectValue /></SelectTrigger>
              <SelectContent>{PLAY_ICONS.map(p => <SelectItem key={p.id} value={p.id}>{p.label}</SelectItem>)}</SelectContent>
            </Select>
            <Button onClick={addPlay}><Plus className="w-4 h-4 mr-1"/>Add</Button>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
            {plays.map(pl => (
              <button key={pl.id} className="p-3 rounded-2xl border hover:shadow" onClick={() => setYardageDialog({ open: true, play: pl, yards: '0' })}>
      ...
      <Card className="mb-4">
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-2 mb-3">
            <Input placeholder="Opponent name" value={newOpponent} onChange={e => setNewOpponent(e.target.value)} />
            <Input type="date" value={newDate} onChange={e => setNewDate(e.target.value)} />
            <Button onClick={addGame}>Create</Button>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-2">
            {games.map(g => (
              <button key={g.id} onClick={() => setActiveGameId(g.id)} className={`px-3 py-2 rounded-xl border ${activeGameId === g.id ? "bg-neutral-100" : "bg-white"}`}>
                <div className="text-sm font-medium">vs {g.opponent}</div>
                <div className="text-xs text-muted-foreground">{format(new Date(g.game_date + "T00:00:00"), "MMM d, yyyy")}</div>
              </button>
            ))}
          </div>
        </CardContent>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><LineChart className="w-4 h-4" />Recent (last 10)</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-2">
              {recentCalls.length === 0 && <div className="text-sm text-muted-foreground">No data yet.</div>}
              {recentCalls.map(rc => (
                <div key={rc.id} className="flex items-center justify-between text-sm border rounded-xl p-2">
                  <div>
                    <div className="font-medium leading-tight">{rc.play_name}</div>
                    <div className="text-xs text-muted-foreground">{format(new Date(rc.created_at), "h:mma")}</div>
                  </div>
                  <div className={`font-semibold ${rc.yards >= 0 ? "" : "text-red-600"}`}>{rc.yards} yds</div>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Per-play Avg (recent)</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-2">
              {perPlayStats.length === 0 && <div className="text-sm text-muted-foreground">No stats yet.</div>}
              {perPlayStats.map(s => (
                <div key={s.name} className="flex items-center justify-between border rounded-xl p-2 text-sm">
      ...
      {/* Yardage dialog */}
      <Dialog open={yardageDialog.open} onOpenChange={(o) => setYardageDialog(d => ({ ...d, open: o }))}>
        <DialogContent>
          <DialogHeader><DialogTitle>Record Yards — {yardageDialog.play?.name}</DialogTitle></DialogHeader>
